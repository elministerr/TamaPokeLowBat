...
| `test_audio.cpp` | arranque silencioso durmiendo, migracion y persistencia del volumen, niveles PCM distintos y cancelacion de efectos al silenciar/dormir; ejecuta audio.cpp contra GPIO/I2S/colas simulados |
| `test_cry.cpp` | lectura WAV y metadatos, rechazo de archivos corruptos/truncados, grito por especie, escalado del volumen, toques repetidos, cola llena, fallback y cancelacion durante la reproduccion; usa archivos SD e I2S simulados |
...
Lo que necesita la placa fisica: pantalla y tactil (`TamaPoke.ino`), calidad del
sonido/cableado del codec, tarjeta SD (`sdmon.cpp`) y RTC/bateria (`rtcbat.cpp`). El
