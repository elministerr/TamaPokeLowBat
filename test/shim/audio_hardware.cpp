...
struct AudioQueue { unsigned capacity; std::deque<uint8_t> items; };
...
QueueHandle_t xQueueCreate(unsigned length, unsigned) {
  queues.emplace_back(new AudioQueue{length, {}});
...
int xQueueSend(QueueHandle_t handle, const void *item, uint32_t) {
  ...
  queue.items.push_back(*static_cast<const uint8_t *>(item));
  return pdPASS;
}
int xQueueReceive(QueueHandle_t handle, void *item, uint32_t) {
  ...
  *static_cast<uint8_t *>(item) = queue.items.front();
